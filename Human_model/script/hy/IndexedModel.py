...
class IndexedModel:
    ...
    def save_config(cls, config):
        print(f"Saveing config {cls.CONFIG_FILE_LOCATION}")
        config.to_csv(cls.CONFIG_FILE_LOCATION, sep="\t", index=False)
    ...
    def repeat_and_predict(self, model_params, test_ids=None):
        ...
        sample_info = load_sample_info(self.CONFIG_FILE_MODEL_DATA_DIR, 'dev')
        config = self.config
        ...
        model_params.update({
            'pca_params': {
                'from_pca': config['from_pca'],
                'n_pcas': config['n_pcas'],
        ...
        ff.init(zr_exp_dir="..", zr_gam_dir="..")
        X_train = ff.fetch_feature(current_discovery)
        ...
        if test_ids is not None:
            test_ids = test_ids.loc[test_ids.index.difference(current_discovery.index)]
        ...
        return IndexedModelResult(oof_result[config['model_name']])

    def combine_results(self, result_a, result_b):
        _, cutoff1, cutoff2 = self.extract_combine_params()
        return self.merge_with_params(result_a, result_b, {
            'low': cutoff1,
            'high': cutoff2,
            'cutoff_1': cutoff1,
            'cutoff_2': cutoff2,
    ...
    def run_model(self, model_params, test_ids):
        type, cutoff1, cutoff2 = self.extract_combine_params()
        if type is not None:
            sub_models = self.get_sub_models()
            return self.combine_results(
                sub_models[0].run_model(model_params, test_ids),
                sub_models[1].run_model(model_params, test_ids),
            )
        from ExisitsPipelinesZheer import build_exists_pipelines
        from hy.FeatureFactory import FeatureFactory
        from hy.model import train_pipelines, run_model
    ...
    def save_external_oof_result(self, result:IndexedModelResult):
        ...
        result.to_csv(f"{self.__class__.DEFAULT_OOF_PRED_DIR}/{self.get_name()}", sep=',', header=True, float_format="%0.12lf")
    ...
    def repeat_combine_result(self):
        ...
        return self.combine_results(r1, r2)
...
class IndexedLabeledModel:
    ...
    def create_labeled_model_class(cls, round_name)-> Type[IndexedTrainCvModel]:
        return type(f'IndexedLabeledModel{round_name}', (IndexedTrainCvModel,), {
            'DEFAULT_OOF_PRED_DIR': f"./preds_{round_name}",
            'DEFAULT_REPORTS_DIR': f"./reports/{round_name}",
            'CONFIG_FILE_LOCATION': f"./selected_model_{round_name}.csv",
